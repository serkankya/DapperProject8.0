...
namespace Project.WebAPI.Repositories.VehicleRepository
{
	public class VehicleRepository : IVehicleRepository
	{
		...
		public async Task<List<ResultVehicleAmenitiesDto>> ListVehicleAmenities(int id)
		{
			string getAmenitiesQuery = "SELECT va.VehicleAmenityId, va.VehicleId, va.AmenityId, va.Status, a.Title, a.Status FROM VehicleAmenities va INNER JOIN Amenities a ON a.AmenityId = va.AmenityId WHERE va.VehicleId = @vehicleId AND va.Status = 1 AND a.Status = 1";
			...
			using (var connection = _dapperContext.CreateConnection())
			{
				var values = await connection.QueryAsync<ResultVehicleAmenitiesDto>(getAmenitiesQuery, parameters);
				return values.ToList();
			}
		}
		...
		public async Task InsertVehicle(InsertVehicleDto insertVehicleDto)
		{
			...
			string insertVehicleDetailsQuery = "INSERT INTO VehicleDetails (VehicleId, EngineType, Transmission, FuelType, Mileage, NumberOfSeats, Description) VALUES (@vehicleId, @engineType, @transmission, @fuelType, @mileage, @numberOfSeats, @description); SELECT SCOPE_IDENTITY();";

			string insertVehicleAmenities = "INSERT INTO VehicleAmenities (VehicleId, AmenityId) VALUES (@vehicleId, @amenityId)";

			string insertVehicleImages = "INSERT INTO VehicleImages (VehicleId, ImageUrl) VALUES (@vehicleId, @imageUrl)";
			...
			using (var connection = _dapperContext.CreateConnection())
			{
				...
				using (var transaction = connection.BeginTransaction())
				{
					try
					{
						...
						//table of VehicleAmenities
						foreach (var amenityId in insertVehicleDto.AmenityId!)
						{
							var vehicleAmenityParameters = new DynamicParameters();

							vehicleAmenityParameters.Add("@vehicleId", insertedVehicleId);
							vehicleAmenityParameters.Add("@amenityId", amenityId);

							await connection.ExecuteAsync(insertVehicleAmenities, vehicleAmenityParameters, transaction);
						}

						//table of VehicleImages
						foreach (string imageUrl in insertVehicleDto.ImageUrl!)
						{
							var vehicleImagesParameters = new DynamicParameters();

							vehicleImagesParameters.Add("@vehicleId", insertedVehicleId);
							vehicleImagesParameters.Add("@imageUrl", imageUrl);

							await connection.ExecuteAsync(insertVehicleImages, vehicleImagesParameters, transaction);
						}
		...
		{
			string listActivesQuery = "SELECT v.VehicleId, v.CategoryId, v.ModelId, m.ModelName, b.BrandName, c.CategoryName, v.LicensePlate, v.Year, v.Color, v.CoverImage, v.PricePerDay, v.InsertedDate, v.Status, vD.EngineType, vD.Transmission, vD.FuelType, vD.Mileage, vD.NumberOfSeats, vD.Description FROM Vehicles v INNER JOIN VehicleDetails vD ON v.VehicleId = vD.VehicleId INNER JOIN Models m ON v.ModelId = m.ModelId INNER JOIN Brands b ON b.BrandId = m.BrandId INNER JOIN Categories c ON c.CategoryId = v.CategoryId WHERE v.Status = 1";
		...
		public async Task<List<ResultVehicleImagesDto>> ListVehicleImages(int id)
		{
			string listVehicleImages = "SELECT * FROM VehicleImages WHERE VehicleId = @vehicleId";
			...
			using (var connection = _dapperContext.CreateConnection())
			{
				var values = await connection.QueryAsync<ResultVehicleImagesDto>(listVehicleImages, parameters);
				return values.ToList();
			}
		}
